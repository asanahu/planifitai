...
def test_get_me_with_complete_profile(test_client: TestClient, user_with_profile, auth_headers):
...
def test_put_me_profile_update_existing(test_client: TestClient, user_with_profile, auth_headers):
    ...
    response = test_client.put("/api/v1/users/me/profile", json=payload, headers=auth_headers)
...
def test_put_me_profile_validation_errors(test_client: TestClient, user_without_profile):
...
def test_put_me_profile_idempotent(test_client: TestClient, user_with_profile, auth_headers):
    ...
    # First request
    response1 = test_client.put("/api/v1/users/me/profile", json=payload, headers=auth_headers)
    assert response1.status_code == 200
    
    # Second identical request
    response2 = test_client.put("/api/v1/users/me/profile", json=payload, headers=auth_headers)
